...
programs.zsh = {
    ...
    initContent = ''
      ...
      # %F{7} делает скобки и тильду светло-серыми (c8c8c8)
      # %F{15} делает имя soundwave чистым белым (ffffff)
      # %n — имя пользователя, %~ — текущая папка (сокращается до ~)
      # %f — сброс цвета, чтобы ввод команды не красился цветом промпта
      PROMPT='%F{7}[%F{15}%n%F{7}:%F{15}%~%F{7}]%F{15}$(git_prompt_info)%f '
    '';
  };
}
